...
class detectv5:
	def __init__(self, model, imgs_w,imgs_h, conf_threshold,score_threshold,nms_threshold, classes_txt):
	...
	def detection(self, net, classes): 
		while True:
			ret, img = self.vid.read()
			blob = cv2.dnn.blobFromImage(
				img, 1/255 , 
				(self.img_w, self.img_h), 
				swapRB=True, mean=(0,0,0), 
				crop= False
			)
			net.setInput(blob)
			t1= time.time()
...
if __name__ == "__main__":
	...
	parser.add_argument('--weights', default='./weight/yolov5s.onnx', type=str, help='model weights path')
	...
	parser.add_argument('--classes',type=str,default='./classes/classes.txt', help='class names')
